...
| `NEMOCLAW_SKIP_SLACK_AUTH_VALIDATION` | `1`, `true`, `yes`, or `on` to enable | Skips the live Slack `auth.test` and `apps.connections.open` credential probes during onboard and `channels add slack`. Use only in restricted networks or hermetic test environments; Slack token format checks still apply. Reserved test-placeholder tokens (`xoxb-fake-*`, `xoxb-test-*`, `xapp-fake-*`, `xapp-test-*`) skip the live probes automatically without this flag, so the `slack` policy preset is applied for `policy-list` testing without a real Slack workspace. |
