...
Hermes stores session history in a SQLite database, so positional keys, `--agent`, `--format tar`, and `--include-trajectory` are OpenClaw-only and are refused with a clear error when the sandbox is Hermes.
...
| Flag | Description |
|------|-------------|
| `--agent <id>` | OpenClaw only. Agent id when `<keys>` are aliases rather than the canonical `agent:<id>:<rest>` form. |
