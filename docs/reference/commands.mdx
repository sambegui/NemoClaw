...
Hermes stores session history in a SQLite database, so the command refuses positional keys, `--agent`, `--format tar`, and `--include-trajectory` with a clear error when the sandbox is Hermes.
