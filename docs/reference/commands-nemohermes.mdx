...
For browser access to the dashboard, use `nemohermes <name> dashboard-url`; Hermes dashboard auth is read from the in-sandbox config (`~/.hermes/config.yaml`), not a gateway token.
