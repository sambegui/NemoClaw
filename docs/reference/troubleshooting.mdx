...
Current NemoClaw stores SHA-256 hashes for messaging credentials in the sandbox's messaging plan credential bindings.
It uses those hashes to detect when a token has changed.
