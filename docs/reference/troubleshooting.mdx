...
`nemoclaw <sandbox> shields status` also cross-checks the filesystem: when the contract is broken it reports `NOT CONFIGURED (default mutable state) — but config permissions are DRIFTED` and exits non-zero instead of claiming a clean mutable state.
