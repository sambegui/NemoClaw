...
| Channel credentials (Slack, Discord, Telegram, Teams, and other supported channels) | Rebuild required (channel configuration is baked into the sandbox image; secrets stay in OpenShell providers or host-side credential storage) | `$$nemoclaw <name> channels add <channel>` then accept the rebuild prompt |
