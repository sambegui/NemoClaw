...
if echo "$STATUS_OUTPUT" | grep -q "DRIFTED"; then
  fail "clean lockdown should not report DRIFTED: ${STATUS_OUTPUT}"
else
  pass "shields status does not report DRIFTED on a clean lockdown"
fi

# ══════════════════════════════════════════════════════════════════
# Phase 5b: host-root tamper — shields status surfaces drift, shields up re-locks
# ══════════════════════════════════════════════════════════════════
section "Phase 5b: shields status detects host-root tamper"

CTR=$(docker ps --filter "name=openshell-${SANDBOX_NAME}" --format "{{.Names}}" | head -n 1)
if [ -z "${CTR}" ]; then
  fail "Could not find sandbox container openshell-${SANDBOX_NAME} for tamper drill"
else
  ...
  if [ "${TAMPER_EXIT}" != "0" ]; then
    ...
    info "Re-locking via shields up to exercise the documented recovery path"
    RELOCK_OUTPUT=$(nemoclaw "${SANDBOX_NAME}" shields up 2>&1)
    RELOCK_EXIT=$?
    echo "$RELOCK_OUTPUT"

    if [ "${RELOCK_EXIT}" = "0" ]; then
      pass "shields up exits 0 after re-lock from drift"
    else
      fail "shields up should exit 0 after re-lock, got ${RELOCK_EXIT}: ${RELOCK_OUTPUT}"
    fi

    if echo "$RELOCK_OUTPUT" | grep -q "Lockdown re-applied"; then
      pass "shields up reports drift remediation"
    else
      fail "shields up should report drift remediation: ${RELOCK_OUTPUT}"
    fi
    ...
    if [ "${CLEAN_EXIT}" = "0" ] && echo "$CLEAN_OUTPUT" | grep -q "Shields: UP (lockdown active)"; then
      pass "shields status returns to clean UP after shields up re-lock"
    else
