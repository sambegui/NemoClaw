...
export default class SandboxSessionsExportCommand extends NemoClawCommand {
  ...
  static description = [
    ...
    "staging path, then download the resulting single JSONL stream to the host.",
    "Hermes stores the session history in a SQLite database, so positional keys,",
    "--agent, --format tar, and --include-trajectory are OpenClaw-only and are",
    "rejected with a clear error when the sandbox is Hermes. The host destination",
    "defaults to ./sessions-<sandbox>.jsonl; --out picks a different path.",
    "",
