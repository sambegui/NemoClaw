...
import { buildGatewayPreloadSelfHealLines } from "./runtime-recovery-preload";
...
export function buildOpenClawRecoveryScript(port: number): string {
  ...
  return [
    ...
    "[ -f ~/.bashrc ] && . ~/.bashrc;",
    'if [ "$_PE_MISSING" = "0" ]; then',
    ...buildGatewayPreloadSelfHealLines(),
    "fi;",
    'if [ "$_PE_MISSING" = "0" ]; then case "${NODE_OPTIONS:-}" in *"--require /tmp/nemoclaw-sandbox-safety-net.js"*) _SN_MISSING=0 ;; *) _SN_MISSING=1 ;; esac; case "${NODE_OPTIONS:-}" in *"--require /tmp/nemoclaw-ciao-network-guard.js"*) _CIAO_MISSING=0 ;; *) _CIAO_MISSING=1 ;; esac; if [ "$_SN_MISSING" = "0" ] && [ "$_CIAO_MISSING" = "0" ]; then _GUARDS_MISSING=0; else _GUARDS_MISSING=1; fi; else _GUARDS_MISSING=0; fi;',
